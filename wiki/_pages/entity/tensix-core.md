...
Data flows through a dual Network-on-Chip (NoC) interface into the core. The NoC operates in a quasi-full-duplex configuration: NoC 0 and NoC 1 traverse the chip in opposite directions in a wraparound 2D torus topology, using X/Y-major per-tile routing, enabling bidirectional communication with unidirectional links while optimizing power and area. Typical data flow: NoC 0 reads data from DRAM or other cores; the Unpacker reformats data into 32×32 tiles; the Matrix/Vector units compute; the Packer compresses results; and NoC 1 sends results to DRAM or other cores. The matrix unit (FPU) operates at 19-bit precision for high-throughput multiply-accumulate; the vector unit (SFPU) provides 32 lanes of FP32 compute. The 1.5 MB (1464 KiB) of local L1 SRAM is organized into 16 banks for parallel access. A MOP (micro-op) expander can expand a single issued instruction into thousands of micro-operations, reducing instruction-dispatch overhead for repetitive tile operations; a Wait Gate synchronizes instructions across the pipeline via hardware semaphores. Programming a Tensix Core typically requires three kernel types per core: a reader kernel for data input, a compute kernel for calculations, and a writer kernel for data output, which coordinate through circular buffers in the SRAM. [[TT-Metalium]] is the programming model that orchestrates these reader, compute, and writer kernels mapped to Tensix cores.
...
- It includes a matrix engine (FPU, 19-bit precision), a vector engine (SFPU, 32 lanes of FP32), unpack/pack units, and 1.5 MB (1464 KiB, 16 banks) of software-managed local L1 SRAM, with no cache hierarchy.
...
- The NoC operates in a quasi-full-duplex configuration: two unidirectional wraparound rings (NoC 0 and NoC 1) traverse opposite directions using X/Y-major per-tile routing, enabling simultaneous send and receive while optimizing power and area.
...
- A MOP expander turns a single issued instruction into thousands of micro-operations; a Wait Gate synchronizes pipeline stages via hardware semaphores.
